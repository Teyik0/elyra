...
// Module-level regex constants for test assertions
...
const REFRESH_REG_ELYSION_PAGE_RE = /\$RefreshReg\$\s*\(\s*\w*ElysionPage/;
...
describe("page() component extraction", () => {
  ...
  test("registers the extracted component with React Refresh", () => {
    ...
    expect(result).toMatch(REFRESH_REG_ELYSION_PAGE_RE);
  });
