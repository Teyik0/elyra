...
export const HMR_INTERNAL_PORT = 24_678;
...
export async function createVitePlugin(pagesDir: string) {
  const vite = await createViteServer({
    plugins: [elysionPlugin({ pagesDir })],
    server: {
      middlewareMode: true,
      hmr: {
        // Do NOT pass `server` — Bun's Server ≠ Node http.Server.
        ...
        // Tell /@vite/client to open ws://localhost:<clientPort>/__vite_hmr.
    ...
    new Elysia()
      ...
      .onRequest(({ request }) => {
        ...
        if (shouldHandleByVite(pathname)) {
          ...
          // toFetchResponse(res) returns res.fetchResponse — the Promise<Response>
          // that resolves when res.end() is called by Vite (or our next() above).
