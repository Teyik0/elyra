...
/** Adapter that turns `DefaultErrorScreen` into an `ErrorComponent` shape. */
export const DefaultErrorFallback: ErrorComponent = ({ error, reset }) => (
  <DefaultErrorScreen digest={error.digest} message={error.message} reset={reset} />
);

/** Adapter that turns `DefaultNotFoundScreen` into a `NotFoundComponent` shape. */
export const DefaultNotFoundFallback: NotFoundComponent = ({ error }) => (
