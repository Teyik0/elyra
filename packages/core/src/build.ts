...
function patternToTypeString(pattern: string): string {
...
/**
 * Generates .elysion/routes.d.ts — augments RouteManifest in elysion/link
 * so that <Link href="..."> has typesafe autocompletion.
 *
 * Users must include ".elysion" in their tsconfig.json "include" array once.
 */
function writeRouteTypes(routes: ResolvedRoute[], outDir: string): void {
  const hrefs = routes.map((r) => patternToTypeString(r.pattern)).join(" | ");
  const content = `// Auto-generated by Elysion. Do not edit manually.
// Add ".elysion" to your tsconfig.json "include" array to enable typed hrefs.
import "elysion/link";
