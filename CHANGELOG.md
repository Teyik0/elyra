...
- **Sync bundle isolation coverage** — build fixtures verify that core, PostgreSQL, Redis, hybrid, and browser bundles contain only their imported backends; optional adapters declare `sideEffects: false` and are not root dependencies.
...
### Changed
- **Production sync requires distributed storage** — implicit memory remains zero-config in development, while production rejects missing or process-local adapters. Pass the same `{ adapter, notifier? }` object to `furinSync()` and `furin({ sync })`.
