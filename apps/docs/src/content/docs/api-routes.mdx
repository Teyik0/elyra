...
Production requires an explicit distributed adapter. Install only the backend you use; the root package neither depends on nor re-exports either adapter:
...
Both optional packages declare `sideEffects: false` and use type-only imports from Furin. PostgreSQL contains no Redis implementation, Redis contains no PostgreSQL migration, and neither backend is present in the browser bundle. IndexedDB outbox/offline replay is tracked separately and is not part of this server contract.
