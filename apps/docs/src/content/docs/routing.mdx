...
Furin generates `furin-env.d.ts` so `<Link />`, `useSearch`, and `useSetSearch` are fully typed from the route manifest.
...
The same types drive `useSearch` and `useSetSearch` from `@teyik0/furin/search`:
