...
bunx --package @teyik0/furin-sync-postgres furin-sync-postgres-migrate
```
...
For low-latency cross-replica wake-ups, install `@teyik0/furin-sync-redis` and add a notifier using the same namespace. Create the Redis client once and replace the `sync` declaration above with:
