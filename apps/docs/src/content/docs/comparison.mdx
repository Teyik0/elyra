...
| **On-demand cache invalidation** | `revalidatePath`, `revalidateTag`, `updateTag` | Coordinate router/query cache in application code | `revalidatePath`, `revalidateTag`, typed route invalidations |
| **Durable idempotency journal** | Application responsibility | Application responsibility | PostgreSQL or Redis adapter |
...
| **Optional backend isolation** | Deployment/runtime dependent | Environment code is build-time separated | Separate side-effect-free adapter packages |
