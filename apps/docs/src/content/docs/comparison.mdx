...
| **Cache invalidation by tag** | ✅ (`revalidateTag`) | ❌ | ✅ (`revalidateTag`, `furinInvalidate`) |
...
| **Typed search read / update** | ❌ (manual `useSearchParams`) | ✅ (`useSearch`, `useSearchParams`) | ✅ (`useSearch("/products")`, `useSetSearch("/products")`) |
