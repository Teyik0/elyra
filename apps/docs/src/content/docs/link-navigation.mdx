...
## `useSearch` and `useSetSearch`

Read and update the current route's query params with full type safety.

```tsx
import { useSearch, useSetSearch } from "@teyik0/furin/search";
...
function ProductList() {
  ...
  return (
    <div>
      ...
      <button
        onClick={() => setSearch({ page: search.page + 1 }, { replace: true })}
      >
        Next page
      </button>
    </div>
...
Returns the server-resolved query object for the current route. The type is inferred from the `query` schema declared on the route chain (`createRoute({ query })` in layout or page `_route.tsx` files).
...
Returns a function that patches the current search and navigates to the updated URL. The patch is shallow-merged with the existing query params.
...
setSearch((prev) => ({ page: prev.page + 1 }), { replace: false });
```
