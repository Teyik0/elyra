...
**Client side** — disabled by default. The generated hydration entry uses a no-op logger unless you enable `clientLogging`. Keeping it off avoids adding `evlog` and `evlog/http` to the browser bundle.
...
Enable browser-side logging with `clientLogging: true`. This adds `evlog` and `evlog/http` to the browser bundle, then points the generated hydration entry at `/_furin/ingest`.
