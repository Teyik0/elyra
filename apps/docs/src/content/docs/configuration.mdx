...
Enable Furin's built-in sync engine. Furin mounts the internal SSE stream at `/_furin/sync`, injects the client runtime automatically, and lets API routes opt into mutation sync with `furinSync()`.
