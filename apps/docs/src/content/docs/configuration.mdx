...
`sync: true` uses process-local memory in development. Production rejects implicit or process-local storage and requires a distributed adapter. Use the same runtime object with `furinSync(sync)` and `furin({ sync })`; this keeps mutation reservation, replay, SSE, and catch-up on one namespace.
...
Neither package is a dependency of `@teyik0/furin`; install only the backend you import.
