...
**Type:** `boolean | { adapter: SyncAdapter; notifier?: SyncNotifier; streamPath?: string }`
...
`sync: true` uses process-local memory in development. Production rejects implicit or process-local storage and requires an explicit durable adapter. A `host-local` SQLite adapter is valid for one machine; multi-host replicas require PostgreSQL or Redis. Use the same runtime object with `furinSync(sync)` and `furin({ sync })`; this keeps mutation reservation, replay, SSE, and catch-up on one namespace.
...
The adapters are separate, tree-shakable packages:
...
- `@teyik0/furin-sync-sqlite` — host-local adapter using `bun:sqlite`, used by the task-manager example.

No adapter package is a dependency of `@teyik0/furin`; install only the backend you import.
