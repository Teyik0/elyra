...
The `invalidate` route option is provided by the `furinInvalidate()` macro. Install the macro on the same Elysia plugin that declares the routes using `invalidate`.
