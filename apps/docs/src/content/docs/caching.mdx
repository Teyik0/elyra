...
In development, the default journal and idempotency responses live in process memory. Production requires an explicit durable adapter. PostgreSQL completion atomically stores the replay response, advances the cursor, and records invalidations in one transaction. Lease acquisition, the application handler, and domain writes happen separately. Redis provides the equivalent completion contract with Lua and Redis Streams, while SQLite provides it for one host. SSE and `SyncNotifier` remain best-effort wake-ups, while `/_furin/sync/changes` is the durable recovery path.
