...
The `sync` route option is provided by the `furinSync()` macro. Install the macro on the same Elysia plugin that declares the routes using `sync`.

`furinSync()` also requires an `Idempotency-Key` header for every synced mutation and broadcasts successful invalidations to browsers connected to `/_furin/sync`. This makes idempotency and multi-tab refresh part of the default mutation contract instead of per-route boilerplate.
...
When `furin({ sync: true })` is enabled, Furin also mounts `/_furin/sync` and injects the stream configuration into the page HTML. The client runtime opens an SSE connection automatically and listens for `furin.invalidate` events. When another tab, device, or client performs a synced mutation, the current page's prefetched HTML is invalidated and the active route is refreshed when it depends on the changed path or tag.
