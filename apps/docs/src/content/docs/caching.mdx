...
In development, the default journal and idempotency responses live in process memory. Production requires a distributed adapter. PostgreSQL stores mutation leases, replay responses, cursors, and invalidations in one transaction; Redis provides the same contract with Lua and Redis Streams. SSE and `SyncNotifier` remain best-effort wake-ups, while `/_furin/sync/changes` is the durable recovery path.
