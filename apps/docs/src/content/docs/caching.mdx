...
`furinSync()` requires an `Idempotency-Key` header for every synced mutation, replays successful responses for duplicate requests, and appends successful invalidations to its change log. This makes idempotency and multi-tab refresh part of the default mutation contract instead of per-route boilerplate.
...
The current journal and idempotency responses live in process memory. They support reconnects within one process but do not survive restarts or synchronize multiple replicas.
